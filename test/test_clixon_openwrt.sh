...
# Example: test_clixon_openwrt.sh distclean
# May need prereqs, eg
#   sudo apt install libncurses-dev
set -eu

# This script does things in the current dir
rundir=$(pwd)
...
# openwrt directory
openwrtdir=${rundir}/openwrt

# This is the local feed dir (where the wifi application is defined)
localfeeddir=${rundir}/tmplocalfeed
...
function usage()
{
    echo "usage: checkout  : checkout and build openwrt from scratch + start vm and run test
       distclean : build openwrt from scratch + start vm and run test
       vbox      : start vm and run test
       test      : run test"
}
...
# Verb: what to do
what=$1
if [ $what != checkout -a $what != distclean -a $what != vbox -a $what != test ]; then
    usage
    exit 255
fi
